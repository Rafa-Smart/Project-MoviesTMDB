...
document.addEventListener("click", async function (e) {
  if (e.target.className.includes("button-page")) {
    let data = await ambilDataPopular(e.target.dataset.page);
    tampilData(data);
    nambahPageKelayar();
  }
});
...
function nambahPageKelayar() {
  let container = document.querySelector(".container-popular");
  let pagenya = page();
...
function ambilDataPopular(page = null) {
  const confignya = konfigurasianjay();
  let url = `https://api.themoviedb.org/3/movie/popular?page=${
    page ? page : 1
  }`;
