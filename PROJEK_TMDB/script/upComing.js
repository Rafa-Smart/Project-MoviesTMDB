...
document.addEventListener("DOMContentLoaded", async function () {
  const datadariapi = await ambilDataupComing();
...
document.addEventListener("click", async function (e) {
  if (e.target.className.includes("button-page")) {
    let data = await ambilDataupComing(e.target.dataset.page);
    tampilData(data);
    nambahPageKelayar();
  }
});
...
function nambahPageKelayar() {
  let container = document.querySelector(".container-upComing");
  let pagenya = page();
...
function ambilDataupComing(page = null) {
  const confignya = konfigurasianjay();
  let url = `https://api.themoviedb.org/3/movie/upcoming?page=${
    page ? page : 1
  }`;
